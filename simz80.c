 ...
 * 
 * These commands were not selecting the correct register to output
        case 0x41:             OUT (C),B 
        case 0x49:             OUT (C),C 
        case 0x51:             OUT (C),D 
        case 0x79:             OUT (C),A 
...
dfd_prefix(FASTREG IXY)
{
    ...
    FASTWORK temp, adr, acu, op, sum, cbits;
		...
		default: PC--;		/* ignore DD */
            printf("unknown ix iy command %2.2X\n",op);
            exit (1);
		}
    SAVE_STATE();
    return(IXY);
...
//#endif
    ...
    /*
    The single step function works by setting singleStep to 2
        this allows the code to execute the
            ; execute one step of program
            POP  AF
            RETN	; return using address previously added to stack
      */
      // checks if single step set
      if (singleStep != 0 ){
          // see if time to set NMI
          singleStep -= 1;
          if (singleStep == 0){
              NMI_flag=1;
          }
      }
      // not doing other INTs
...
//            printf("count %d\n",count);
	...
	case 0xED:			/* ED prefix */
		...
		case 0xB1:			/* CPIR */
			acu = hreg(AF);
			BC &= 0xffff;
			// fix DA - if B is 0 then should do loads cycles
			if (BC == 0)
			    BC = 0x10000;
			do {
				temp = GetBYTE(HL); ++HL;
		...
		case 0xB2:			/* INIR */
			temp = hreg(BC);
			// fix DA - if B is 0 then should do 256 cycles
			if (temp == 0)
			    temp = 0x100;
			do {
				PutBYTE(HL, Input(lreg(BC))); ++HL;
		...
		case 0xB8:			/* LDDR */
			BC &= 0xffff;
			// fix DA - if BC is 0 then should do loads of cycles
			if (BC == 0)
			    BC = 0x10000;
			do {
				acu = GetBYTE(HL); --HL;
